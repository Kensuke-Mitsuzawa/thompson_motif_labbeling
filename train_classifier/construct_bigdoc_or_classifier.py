...
def cleanup_class_stack(doc,stop):
    """
...
def generate_document_instances(doc,filepath,alphabet_label_list,dutch_training_map,args):
    """
    文書単位で訓練事例を作成する
...
def generate_sentence_instances(doc,filepath,alphabet_label_list,dutch_training_map,args):
    """
    文単位で訓練事例を作成する
...
def load_dfd(training_map,args):
    ...
    for fileindex,filepath in enumerate(dfd_f_list):
        ...
        if args.ins_range=='document':
            dfd_training_map=generate_document_instances(doc,filepath,alphabet_label_list,dfd_training_map,args);
        elif args.ins_range=='sentence':
            #arowを用いた半教師あり学習のために文ごとの事例作成を行う
            dfd_training_map=generate_sentence_instances(doc,filepath,alphabet_label_list,dfd_training_map,args);                
...
def load_tmi(training_map,args):
    ...
    for filepath in tmi_f_list:
        ...
        tokens_set_stack=cleanup_class_stack(doc, args.stop);
        if alphabet_label_list[0] in training_map:
            training_map[alphabet_label_list[0]]+=tokens_set_stack;
        else:
            training_map[alphabet_label_list[0]]=tokens_set_stack;
            tmi_training_map[alphabet_label_list[0]]=tokens_set_stack;
...
def load_dfd_orig(training_map,args):
    ...
    #原文のオランダ語で評価をする
    dutch_original_map={};
    dutch_original_map=original_dutch_module.main(dutch_original_map);
   
    if args.tfidf==False:
    #------------------------------------------------------------ 
    #素性をunigram素性にする
        for label in dutch_original_map:
            tokens_set_stack=dutch_original_map[label];
            #文字情報の素性関数を作成する
            #feature_map_character=make_feature_set(feature_map_character,
            #                                       label, tokens_set_stack, 'dutch_original', stop, args);
    #------------------------------------------------------------ 
    #training_map['dutch_original']=dutch_original_map;   
